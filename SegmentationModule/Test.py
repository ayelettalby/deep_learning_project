...
def dataloader_8(test_folder,batch_size):
    test_dataset_spleen = Seg_Dataset('spleen', test_folder + '/Spleen/Test',
                                       test_folder + '/Spleen/Test_Labels', 2)

    test_dataset_prostate = Seg_Dataset('prostate', test_folder + '/Prostate/Test',
                                      test_folder + '/Prostate/Test_Labels', 2)


    test_dataset_pancreas = Seg_Dataset('pancreas', test_folder  + '/Pancreas/small_Test',
                                         test_folder  + '/Pancreas/small_Test_Labels', 2)

    test_dataset_lits = Seg_Dataset('lits', test_folder + '/Lits/small_Test',
                                      test_folder + '/Lits/small_Test_Labels', 2)

    test_dataset_left_atrial = Seg_Dataset('left_atrial', test_folder + '/Left Atrial/Test',
                                    test_folder + '/Left Atrial/Test_Labels', 2)

    test_dataset_hepatic_vessel = Seg_Dataset('hepatic_vessel', test_folder + '/Hepatic Vessel/small_Test',
                                           test_folder + '/Hepatic Vessel/small_Test_Labels', 2)

    test_dataset_brain = Seg_Dataset('brain', test_folder + '/BRATS/Test',
                                              test_folder + '/BRATS/Test_Labels', 2)


    test_dataset_list = [test_dataset_lits,test_dataset_brain, test_dataset_spleen, test_dataset_prostate, test_dataset_pancreas,
                          test_dataset_left_atrial, test_dataset_hepatic_vessel]
    #test_dataset_list = [test_dataset_pancreas]
...
def test(setting_dict,test_folder, exp_ind,model):
    ...
    test_dataloader=dataloader_8(test_folder,batch_size)
    print ('Data ready')

    test_loss_tot_cur = []  ##cross entropy
    test_loss_tot_spleen_cur = []
    test_loss_tot_prostate_cur = []
    test_loss_tot_lits_cur = []
    test_loss_tot_brain_cur = []
    test_loss_tot_pancreas_cur = []
    test_loss_tot_hepatic_vessel_cur = []
    test_loss_tot_left_atrial_cur = []

    test_organ_dice_tot_cur = []
    test_organ_dice_spleen_cur = []
    test_organ_dice_prostate_cur = []
    test_organ_dice_lits_cur = []
    test_organ_dice_brain_cur = []
    test_organ_dice_pancreas_cur = []
    test_organ_dice_hepatic_vessel_cur = []
    test_organ_dice_left_atrial_cur = []

    test_background_dice_tot_cur = []
    test_background_dice_spleen_cur = []
    test_background_dice_prostate_cur = []
    test_background_dice_lits_cur = []
    test_background_dice_brain_cur = []
    test_background_dice_pancreas_cur = []
    test_background_dice_hepatic_vessel_cur = []
    test_background_dice_left_atrial_cur = []
    test_tumour_dice_pancreas_cur = []

    test_mean_dice_tot_cur = []
    test_mean_dice_spleen_cur = []
    test_mean_dice_prostate_cur = []
    test_mean_dice_lits_cur = []
    test_mean_dice_brain_cur = []
    test_mean_dice_pancreas_cur = []
    test_mean_dice_hepatic_vessel_cur = []
    test_mean_dice_left_atrial_cur = []

    test_sensitivity_all=[]
    test_sensitivity_spleen = []
    test_sensitivity_prostate = []
    test_sensitivity_lits = []
    test_sensitivity_brain = []
    test_sensitivity_pancreas = []
    test_sensitivity_hepatic_vessel = []
    test_sensitivity_left_atrial = []

    test_specificity_all=[]
    test_specificity_spleen = []
    test_specificity_prostate = []
    test_specificity_lits = []
    test_specificity_brain = []
    test_specificity_pancreas = []
    test_specificity_hepatic_vessel = []
    test_specificity_left_atrial = []
    ...
    for i, data in enumerate(test_dataloader):
        ...
        if data['task'][0] == 'hepatic_vessel':
            test_organ_dice_hepatic_vessel_cur.append(organ_dice)
            test_background_dice_hepatic_vessel_cur.append(background_dice)
            test_mean_dice_hepatic_vessel_cur.append(mean_dice)
            test_loss_tot_hepatic_vessel_cur.append(loss.item())
            test_sensitivity_hepatic_vessel.append(sensitivity)
            test_specificity_hepatic_vessel.append(specificity)
        if data['task'][0] == 'brain':
            test_organ_dice_brain_cur.append(organ_dice)
            test_background_dice_brain_cur.append(background_dice)
            test_mean_dice_brain_cur.append(mean_dice)
            test_loss_tot_brain_cur.append(loss.item())
            test_sensitivity_brain.append(sensitivity)
            test_specificity_brain.append(specificity)
        if data['task'][0] == 'left_atrial':
            test_organ_dice_left_atrial_cur.append(organ_dice)
            test_background_dice_left_atrial_cur.append(background_dice)
            test_mean_dice_left_atrial_cur.append(mean_dice)
            test_loss_tot_left_atrial_cur.append(loss.item())
            test_sensitivity_left_atrial.append(sensitivity)
            test_specificity_left_atrial.append(specificity)
        ...
        if data['task'][0] == 'prostate':
            test_organ_dice_prostate_cur.append(organ_dice)
            test_background_dice_prostate_cur.append(background_dice)
            test_mean_dice_prostate_cur.append(mean_dice)
            test_loss_tot_prostate_cur.append(loss.item())
            test_sensitivity_prostate.append(sensitivity)
            test_specificity_prostate.append(specificity)
        if data['task'][0] == 'pancreas':
            ...
            #test_tumour_dice_pancreas_cur.append(tumour_dice)
    ...
    brain_test_organ_dice = np.mean(test_organ_dice_brain_cur)
    brain_test_bg_dice = np.mean(test_background_dice_brain_cur)
    brain_test_mean_dice = np.mean(test_mean_dice_brain_cur)
    brain_test_loss = np.mean(test_loss_tot_brain_cur)
    test_sensitivity_brain = np.mean(test_sensitivity_brain)
    test_specificity_brain = np.mean(test_specificity_brain)
    ...
    left_atrial_test_organ_dice = np.mean(test_organ_dice_left_atrial_cur)
    left_atrial_test_bg_dice = np.mean(test_background_dice_left_atrial_cur)
    left_atrial_test_mean_dice = np.mean(test_mean_dice_left_atrial_cur)
    left_atrial_test_loss = np.mean(test_loss_tot_left_atrial_cur)
    test_sensitivity_left_atrial = np.mean(test_sensitivity_left_atrial)
    test_specificity_left_atrial = np.mean(test_specificity_left_atrial)

    hepatic_vessel_test_organ_dice = np.mean(test_organ_dice_hepatic_vessel_cur)
    hepatic_vessel_test_bg_dice = np.mean(test_background_dice_hepatic_vessel_cur)
    hepatic_vessel_test_mean_dice = np.mean(test_mean_dice_hepatic_vessel_cur)
    hepatic_vessel_test_loss = np.mean(test_loss_tot_hepatic_vessel_cur)
    test_sensitivity_hepatic_vessel = np.mean(test_sensitivity_hepatic_vessel)
    test_specificity_hepatic_vessel = np.mean(test_specificity_hepatic_vessel)

    prostate_test_organ_dice = np.mean(test_organ_dice_prostate_cur)
    prostate_test_bg_dice = np.mean(test_background_dice_prostate_cur)
    prostate_test_mean_dice = np.mean(test_mean_dice_prostate_cur)
    prostate_test_loss = np.mean(test_loss_tot_prostate_cur)
    test_sensitivity_prostate = np.mean(test_sensitivity_prostate)
    test_specificity_prostate = np.mean(test_specificity_prostate)
    ...
    pancreas_test_tumour_dice=np.mean(test_tumour_dice_pancreas_cur)
    ...
    print ('brain organ dice: ' ,brain_test_organ_dice, ', brain background dice: ', brain_test_bg_dice)
    print ('brain sensitivity: ', test_sensitivity_brain, ', brain specificity: ', test_specificity_brain)
    print ('brain mean dice: ', brain_test_mean_dice)
    ...
    print('left_atrial organ dice: ', left_atrial_test_organ_dice, ', left_atrial background dice: ', left_atrial_test_bg_dice)
    print('left_atrial sensitivity: ', test_sensitivity_left_atrial, ', left_atrial specificity: ', test_specificity_left_atrial)
    print('left_atrial mean dice: ', left_atrial_test_mean_dice)
    print('hepatic_vessel organ dice: ', hepatic_vessel_test_organ_dice, ', hepatic_vessel background dice: ', hepatic_vessel_test_bg_dice)
    print('hepatic_vessel sensitivity: ', test_sensitivity_hepatic_vessel, ', hepatic_vessel specificity: ', test_specificity_hepatic_vessel)
    print('hepatic_vessel mean dice: ', hepatic_vessel_test_mean_dice)
    print('prostate organ dice: ', prostate_test_organ_dice, ', prostate background dice: ', prostate_test_bg_dice)
    print('prostate sensitivity: ', test_sensitivity_prostate, ', prostate specificity: ', test_specificity_prostate)
    print('prostate mean dice: ', prostate_test_mean_dice)

    print('pancreas organ dice: ', pancreas_test_organ_dice, ', pancreas background dice: ', pancreas_test_bg_dice,
          'pancreas tumour dice: ', pancreas_test_tumour_dice)
    ...
    with open(r'G:\Deep learning\Datasets_organized\small_dataset\Experiments\exp_{}\test_results_exp_{}_epoch_{}.txt'.format(exp_ind,exp_ind,epoch),'w') as f:
        f.writelines('brain organ dice: ' +str(brain_test_organ_dice)+ ', brain background dice: '+ str(brain_test_bg_dice)+
                     '\nbrain sensitivity: '+str(test_sensitivity_brain)+ ', brain specificity: '+
                           str(test_specificity_brain)+
        ...
        '\nleft_atrial organ dice: ' + str(left_atrial_test_organ_dice) + ', left_atrial background dice: ' + str(left_atrial_test_bg_dice) +
        '\nleft_atrial sensitivity: ' + str(test_sensitivity_left_atrial) + ', left_atrial specificity: ' + str(test_specificity_left_atrial) +
        '\nhepatic_vessel organ dice: ' + str(hepatic_vessel_test_organ_dice) + ', hepatic_vessel background dice: ' + str(hepatic_vessel_test_bg_dice) +
        '\nhepatic_vessel sensitivity: ' + str(test_sensitivity_hepatic_vessel) + ', hepatic_vessel specificity: ' + str(test_specificity_hepatic_vessel) +
        '\nprostate organ dice: ' + str(prostate_test_organ_dice) + ', prostate background dice: ' + str(prostate_test_bg_dice) +
        '\nprostate sensitivity: ' + str(test_sensitivity_prostate) + ', prostate specificity: ' + str(test_specificity_prostate) +
