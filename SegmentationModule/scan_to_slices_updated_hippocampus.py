...
def main(path, task_name,end_shape,truncate=False, binary=False):
        ...
        for ind,file in enumerate(files,0):
            if file.endswith('.hdr'):
                if task_name=="BRATS":
                    ...
                    output = np.empty((3,end_shape[1],end_shape[0]), dtype=float, order='C')
                    ...
                    for i in range(1, num_slices+1):
                        ...
                        output_new[1,:,:]=np.flipud(np.rot90(re_sample(data[:,:,i], end_shape))) #middle slice
                        output_new[0,:,:]=np.flipud(np.rot90(re_sample(data[:,:,i-1],end_shape)))#bottom slice
                        output_new[2,:, :]=np.flipud(np.rot90(re_sample(data[:, :, i+1],end_shape))) #top slice
    ...
    return None
############################################
path= r'G:\Deep learning\Datasets_organized\hippocampus' #change to relevant source path
