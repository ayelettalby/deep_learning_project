...
from SettingsTransfer import SegSettings
...
import logging
import sys
from Model_Transfer import Unet_2D as Unet_2D_transfer
import os
...
def save_samp_val_irr(model, epoch, settings):
...
def train_transfer(setting_dict, task,data_folder, exp_ind, model):
    ...
    for epoch in range(0, num_epochs):
         ...
         train_loss_cur = []
         train_organ_dice_cur = []
         train_background_dice_cur = []

         val_loss_cur = []
         val_organ_dice_cur = []
         val_background_dice_cur = []
         ...
         for i,sample in enumerate(train_dataloader,1):
             ...
             train_organ_dice_cur.append(organ_dice)
             train_background_dice_cur.append(background_dice)
             train_loss_cur.append(loss.item())
             if i % 30 == 0:
                 save_samp(sample['image'][0], sample['mask'][0], sample['task'][0], outputs, epoch, i,
                           settings.snapshot_dir, organ_dice)
                 # save_samp(sample['image'][0], sample['mask'][0], sample['task'][0], outputs[0, :, :, :], epoch, i,
                 #           settings.snapshot_dir, organ_dice)


             if (i + 1) % 50 == 0:
                 print('curr train loss: {}  train organ dice: {}  train background dice: {} \t'
                       'iter: {}/{}'.format(np.mean(train_loss_cur),
                                            np.mean(train_organ_dice_cur),
                                            np.mean(train_background_dice_cur),
         ...
         train_organ_dice_tot.append(np.mean(train_organ_dice_cur))
         train_background_dice_tot.append(np.mean(train_background_dice_cur))
         train_loss_tot.append(np.mean(train_loss_cur)) #cross entropy
         ...
         for i, data in enumerate(val_dataloader):
             ...
             if i%10==0:
                 save_samp_validation(data['image'][0], data['mask'][0], data['task'][0], outputs, epoch, i,
                                      organ_dice,settings)
                 # save_samp_validation(data['image'][0], data['mask'][0], data['task'][0], outputs[0, :, :, :], epoch, i,
                 #                      organ_dice, settings)
             val_organ_dice_cur.append(organ_dice)
             val_background_dice_cur.append(background_dice)
             val_loss_cur.append(loss.item())

         val_organ_dice_tot.append(np.mean(val_organ_dice_cur))
         val_background_dice_tot.append(np.mean(val_background_dice_cur))
         val_loss_tot.append(np.mean(val_loss_cur))

         print('End of epoch {} / {} \t Time Taken: {} min'.format(epoch, num_epochs,(time.time() - epoch_start_time) / 60))
         print('train loss: {} val_loss: {}'.format(np.mean(train_loss_tot), np.mean(val_loss_tot)))

         np.save(settings.dice_dir + r'\train_organ_epoch_{}.npy'.format(epoch),
                 np.array(train_organ_dice_tot))
         np.save(settings.dice_dir + r'\val_organ_epoch_{}.npy'.format(epoch),
                 np.array(val_organ_dice_tot))
...
if __name__ == '__main__':
    exp_ind = 2
    with open(r'G:\Deep learning\Datasets_organized\small_dataset\Transfer_exp\Uterus\truncate_0.2\100\exp_2\imagenet\exp_2.json') as json_file:
    ...
    if settings.exp_type=='transfer':
        model.get_encoder('densenet121',weights=
            r'G:\Deep learning\Datasets_organized\small_dataset\Experiments\exp_3_imagenet_final\checkpoint\encoder_exp_3_epoch_14.pt')
        print('model loaded')
