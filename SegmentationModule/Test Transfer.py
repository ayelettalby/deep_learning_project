...
def test(settings,test_folder,task, exp_ind,model):
    model.to(device)
    model = model.double()
    ...
    test_loss_cur = []  ##cross entropy
    test_organ_dice_cur = []
    test_background_dice_cur = []
    test_mean_dice_cur=[]
    test_sensitivity=[]
    test_specificity=[]
    ...
    for i, data in enumerate(test_dataloader):
        ...
        sensitivity=dices[3]
        specificity=dices[4]
        test_organ_dice_cur.append(organ_dice)
        test_background_dice_cur.append(background_dice)
        test_mean_dice_cur.append(mean_dice)
        test_loss_cur.append(loss.item())
        ...
        print('Iteration: {}/{} Test loss: {} '.format(i, total_steps,np.mean(test_loss_cur)))
        print('Test organ dice: {}  Test background dice: {}'.format(
            np.mean(test_organ_dice_cur), np.mean(test_background_dice_cur)))

    test_loss=np.mean(test_loss_cur)
    test_organ_dice=np.mean(test_organ_dice_cur)
    test_mean_dice=np.mean(test_mean_dice_cur)
    test_background_dice=np.mean(test_background_dice_cur)
    test_sensitivity = np.mean(test_sensitivity)
    test_specificity = np.mean(test_specificity)


    with open(settings.simulation_folder+'test_results_exp_{}_epoch_{}.txt'.format(exp_ind,epoch),'w') as f:
        f.writelines('test loss: ' +str(test_loss)+ '\ntest organ dice: '+ str(test_organ_dice)+
                     'test bg dice: '+str(test_background_dice)+', \ntest mean dice: '+str(test_mean_dice)+
                     ', \ntest sensitiviy: '+str(test_sensitivity)+
                        'test speficity'+str(test_specificity))
...
if __name__ == '__main__':
    exp_ind=1
    epoch=9
    with open(r'G:\Deep learning\Datasets_organized\small_dataset\Transfer_exp\Uterus\truncate_0.2\100\exp_2\transfer\exp_2.json') as json_file:
    ...
    model.load_state_dict(torch.load(settings.model_dir + '\exp_2_epoch_19.pt'.format(exp_ind,epoch)))
    print (settings.model_dir)
    #model.eval()
    data_folder=r'G:\Deep learning\Datasets_organized\Prepared_Data\Uterus_truncate_0.2\100'
    task='Uterus'
    test(settings,data_folder, task,exp_ind,model)
